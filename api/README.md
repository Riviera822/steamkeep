...
**All fifteen numeric settings are parsed strictly (WP 3.12).** Twelve take a
...
`VAULT_EVENT_LOG_MAX_BYTES`) and three take a decimal
(`VAULT_WORKER_POLL_SECONDS`, `VAULT_SIZE_CACHE_TTL`, and WP 3.9's
`VAULT_MANIFEST_ORACLE_TIMEOUT`, which goes through the same `_env_float`).
